...
public class svm {
	...
	public static double svm_predict_values(svm_model[] model, svm_node[] x, double[] dec_values)
	{
		int LabelNum = model.length;
		//System.out.println("Predict length : "+LabelNum);
		double min = 0;
		int minid = -1;
		for(int j=0;j<LabelNum;j++){
			if(model[j].param.svm_type == svm_parameter.ONE_CLASS)
			{
				double[] sv_coef = model[j].sv_coef[0];
				double sum = 0;
				for(int i=0;i<model[j].l;i++)
					sum += sv_coef[i] * Kernel.k_function(x,model[j].SV[i],model[j].param);
				//sum -= model[j].rho[0];
				//dec_values[0] = sum;
				if(j == 0){
					min = sum;
				}
				System.out.println("Predict j: "+j+" , sum: "+sum);
		...
		//System.out.println("Predict label : "+minid);
		return minid;
	}
	...
	public static double svm_predict(svm_model[] model, svm_node[] x)
	{
		int nr_class = model[0].nr_class;
		double[] dec_values;
		if(model[0].param.svm_type == svm_parameter.ONE_CLASS ||
				model[0].param.svm_type == svm_parameter.EPSILON_SVR ||
				model[0].param.svm_type == svm_parameter.NU_SVR)
			dec_values = new double[1];
		else
			dec_values = new double[nr_class*(nr_class-1)/2];
		double pred_result = svm_predict_values(model, x, dec_values);
